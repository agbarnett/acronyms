...
# could remove / from common units so that it only gets counted as one word
# e.g., mmol/l

## Section 1: key characters (also uses ones from 99_main_function_titles.R) ##

# remove these words as acronyms from the abstract
bogus.acronyms.abstract = c('WITHDRAWN','CORRIGENDUM','BACKGROUND','INTRODUCTION','OBJECTIVE','METHODS','RESULTS','DISCUSSION','CONCLUSION','IMPORTANCE','UNASSIGNED','SIGNIFICANCE STATEMENT','PRACTICAL APPLICATION','ABSTRACT TRUNCATED AT \\d+ WORDS')
...
other.math.symbols = c('\\≥','+/-' # need to find an abstract with this _ TO DO
...
# remove hyphens for carraige returns in words

# extract the abstract
abstract = indata$abstract[k]
abstract = str_remove_all(abstract, pattern=to.replace) # remove fractions, superscripts, etc 
...
abstract = str_remove_all(abstract, pattern=symbols)

## break the abstract into words
words = str_split(string=abstract, pattern=' ')[[1]]
words = words[words!=''] # remove blank words
# are all or most words in capitals? if they are then ignore this abstract because it's a journal style
...
if(proportion.capital >= 0.5){
  if(length(words)>2 | proportion.capital==1){ # skip if there's two words and one of them is in capitals, e.g., 6551089[pmid]
    this.exclude = data.frame(pmid=indata$pmid[k], date=indata$date[k], type=indata$type[k], reason = 'Abstract in capitals', stringsAsFactors = FALSE)
    to.return$exclude = TRUE
    to.return$this.exclude = this.exclude
    return(to.return) # break early
  }

...
n.check = 300
post = dbeta(p, shape1=n.check+1, shape2=3+1) # posterior
post = post / sum(post) # scale to p 
plot(p, post, xlim=c(0.8,1), type='b')
cat('The 95% confidence interval is ', round(1000*qbeta(0.025, shape1=n.check+1, shape2=1))/1000, 
    ' to ', round(1000*qbeta(0.975, shape1=n.check+1, shape2=1))/1000, '.\n', sep='')
cat('There is a 95% probability that the sensitivity/specificity is at least ', round(1000*qbeta(0.05, shape1=n.check+1, shape2=1))/1000, '.\n', sep='')

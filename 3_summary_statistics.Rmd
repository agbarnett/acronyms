...
if (temporary==TRUE){
  ...
  acronyms.in.titles = filter(acronyms.in.titles, pmid %in% samp)
  excluded = sample_n(excluded, size=12000)
...
tab = mutate(excluded, reason = ifelse(reason == 'Title in capitals (subtitle, start)',"Title in capitals", reason), # combine "title in capitals"
...
```{r excluded.year, fig.width=15, fig.height=8}
to.plot = mutate(excluded, year=as.numeric(format(date, '%Y'))) %>%
...
```{r acronym.length}
freq(acronyms.in.titles$nchar)
```

The median acronym length was `r median(acronyms.in.titles$nchar)` characters, with an inter-quartile range from `r Q1(acronyms.in.titles$nchar)` to `r Q3(acronyms.in.titles$nchar)` characters. The shortest acronym was `r min(acronyms.in.titles$nchar)` characters and the longest was `r max(acronyms.in.titles$nchar)` characters.

### Histogram of acronym length

```{r acronym.length.histogram}
# split by types of acronym
tab = data.frame(table(acronyms.in.titles$nchar))
hplot = ggplot(data=tab, aes(x=Var1, y=Freq))+
  geom_histogram(stat='identity', fill='sky blue')+
...
```{r acronyms.per.paper}
pmid.with = unique(acronyms.in.titles$pmid) # number of papers with at least one acronym 
n.without = nrow(titles) - length(pmid.with) # number without an acronym
# stats for papers with an abstract
counts = group_by(acronyms.in.titles, pmid) %>%
   summarise(n = n()) %>%
   ungroup() %>%
   mutate(n = ifelse(n >= 5, 5, n), # truncate n to five
          nc = factor(n, levels=0:5, labels=c(0:4, '5+'))) 
# add zeros to counts
zeros = data.frame(pmid=999, n=rep(0, n.without)) %>%
   mutate(nc = factor(n, levels=0:5, labels=c(0:4, '5+'))) 
counts = bind_rows(counts, zeros)
freq(counts$nc)
# percent of abstracts with no acronyms
percent.none = round(100 * sum(counts$n==0) / nrow(counts))
```
